...
function startCompanion() {
  return new Promise((resolve,reject)=>{
    ...
    companion=utilityProcess.fork(companionEntry,[],{env:{...process.env,WOODSHED_WEB_URL:UI_ORIGIN,WOODSHED_COMPANION_PORT:'0',WOODSHED_DATA_DIR:path.join(app.getPath('userData'),'library'),WOODSHED_PROCESSOR:processor,WOODSHED_GPU_MANIFEST:path.join(resources,'gpu-runtime.json'),WOODSHED_GPU_CACHE:path.join(app.getPath('userData'),'accelerators'),SSL_CERT_FILE:process.env.SSL_CERT_FILE||path.join(resources,'processor','woodshed-processor','_internal','certifi','cacert.pem'),PATH:path.join(resources,'bin')+path.delimiter+process.env.PATH,TORCH_HOME:path.join(app.getPath('userData'),'models'),MPLCONFIGDIR:path.join(app.getPath('userData'),'matplotlib')},stdio:'pipe'});
