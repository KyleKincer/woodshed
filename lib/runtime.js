...
class Runtime {
  ...
  async _ensureUv(onLog) {
    ...
    onLog('Downloading package manager…');
    ...
    if (!found) throw new Error('Setup download failed. Try again.');
  ...
  async provision(onLog) {
    ...
    if (process.platform === 'darwin') {
      ...
      onLog('Installing stem separator (this may take a while)…');
  ...
  async ensureBeatnet(onLog) {
    ...
    const uv = await this._ensureUv(onLog);
    onLog('Creating environment for beat detection…');
    ...
    onLog('Building beat tracker…');
    await this._run(uv, [...pip, '--no-build-isolation', 'madmom==0.16.1'], onLog);
    onLog('Installing beat detection (this may take a while)…');
    ...
    onLog('Finishing beat detection setup…');
    await this._run(uv, [...pip, '--no-deps', 'BeatNet'], onLog);
    if (!this.hasBeatnet()) throw new Error('Beat detection setup didn\'t finish. Try again.');
