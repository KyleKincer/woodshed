...
function friendlyError(msg) {
  if (/TorchCodec is required|torchcodec/i.test(msg)) {
    return 'Couldn\'t save the separated audio. Try running setup again, or restart the app.';
  }
  if (/ffmpeg/i.test(msg) && /not found|No such file/i.test(msg)) {
    return 'Audio tools aren\'t available. Finish setup first.';
  }
  if (/demucs/i.test(msg) && /not available|finish setup/i.test(msg)) {
    return 'Stem separator isn\'t ready. Finish setup first.';
  }
  // Strip raw tool exit codes and stack traces for a cleaner message.
  const first = msg.split('\n')[0];
  if (/exited with code \d+$/i.test(first)) return 'Something went wrong while processing. Try again.';
  return msg;
}
...
class Processor {
  ...
  async _ytdlpDownload(jobId, target, tmpDir, stageMsg) {
    const ytdlp = this.tool('yt-dlp');
    if (!ytdlp) throw new Error('Download tool isn\'t ready — finish setup first.');
    ...
    if (!wav) throw new Error('Couldn\'t download audio. Check the link and try again.');
  ...
  async _acquireSpotify(jobId, url, tmpDir) {
    ...
    if (spotdl) {
      this._progress(jobId, 'download', 0, 'Getting track from Spotify…');
      const ffDir = this.runtime.ffmpegDir();
      ...
      if (!audio) throw new Error('Couldn\'t get audio from this Spotify link. Try a YouTube link instead.');
    ...
    if (!query) {
      throw new Error('Could not get this Spotify track. Try a YouTube link or search instead.');
    }
  ...
  async _process(job) {
    ...
    if (!demucs) throw new Error('Stem separator isn\'t ready — finish setup first.');
  ...
  async _separate(jobId, audioPath, base, songDir, quality, stemMode, settings) {
    const demucs = this.tool('demucs');
    this._progress(jobId, 'separate', 0, 'Preparing…');
    ...
    await this._run(demucs, args, {
      onLine: (line) => {
        ...
        if (m) {
          ...
          this._progress(jobId, 'separate', overall, 'Separating stems…');
    ...
    if (!staged.length) { try { fs.rmSync(demucsOut, { recursive: true, force: true }); } catch {} ; throw new Error('No stems were produced. Try a different quality preset.'); }
