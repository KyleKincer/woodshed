  ...
  async _fetchMeta(jobId, source, songId, songDir) {
    ...
    try {
      if (source.type === 'file') {
        ...
        const m = { title: base, uploader: 'Local file', duration: ffprobeDuration(this.tool('ffprobe'), source.value), thumbFile };
        emitMeta(m); return m;
      }
      if (source.type === 'spotify') {
        let m = { title: source.value, uploader: 'Spotify', duration: 0, thumbFile: null };
        try {
      ...
      const m = { title: source.value, uploader: '', duration: 0, thumbFile: null };
      emitMeta(m); return m;
    }
  }
  ...
  async _process(job) {
    ...
    try {
      ...
      if (replaceId) {
        ...
        song = {
          id, title: meta.title, uploader: meta.uploader, duration,
