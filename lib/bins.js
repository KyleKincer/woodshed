...
/** Check all three tools; returns { ytdlp, demucs, ffmpeg } with {found, path}. */
function checkDeps() {
  const tools = { ytdlp: 'yt-dlp', demucs: 'demucs', ffmpeg: 'ffmpeg' };
  const result = {};
  for (const [key, bin] of Object.entries(tools)) {
    const p = resolveBin(bin);
    result[key] = { found: !!p, path: p, bin };
  }
  return result;
}
